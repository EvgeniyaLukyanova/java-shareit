...
class ItemServiceImplTest {
    ...
    void getItemsFromSizeNotEmpty() {
        ...
        FromSizePageable page = FromSizePageable.of(from, size, Sort.by("id").ascending());
        List<Item> items = List.of(item);
        //List.of(item).sort(Comparator.comparing(Item::getId));
        Page<Item> pg = new PageImpl<>(items);
        when(repository.findByOwnerId(userId, page)).thenReturn(pg);
        //when(repository.findByOwnerId(any(), any())).thenReturn(pg);
    ...
    void getAvailableItemsFromSizeNotEmpty() {
        ...
        lenient().when(repository.findAvailableItemsByNameDescription(eq(text), eq(page))).thenReturn(pg);
    ...
    //pg.stream().sorted(Comparator.comparing(Item::getId).reversed());
    @Test
    void createComment() {
        Long userId = 1L;
